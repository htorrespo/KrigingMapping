import pickle, numpy, rpy2, os, sys, time, random, collections
...
def KrigingMapPython(inputPath, namegal, genTable, label='Z', limits=[-3, +2], visualize=False,
                      savePDF=True, sizePixelMap=80):
  #
  ([xK, yK, zK, errzK],
   [minZpoints, maxZpoints],
   rangeZmap) = retrieveFrom_RKriging(inputPath, genTable, namegal, label, sizePixelMap, limits)
  #
  if savePDF:
    print "Creating Plot"
    if visualize:
...
def MCerrors(linear_prof_R, log_prof_R, totRealizations, namegal, genTable, rangeKriging, label='Z'):
  ...
  ## extract profiles
  ### LINEAR
    tmpR, tmpVal = radialProfileLin(namegal, pathNick+namegal+'/Kriging/MC_'+label+'/MC'+str(jj)+'/gridKrig_'+label+'.txt',
                                  label=label, datapoints = genTable)
    list_R_MC_lin.append(tmpR)
    list_Val_MC_lin.append(tmpVal)
  #
  ### LOGARITHMIC
    tmpR, tmpVal = radialProfileLog(namegal, pathNick+namegal+'/Kriging/MC_'+label+'/MC'+str(jj)+'/gridKrig_'+label+'.txt',
                                  label=label, datapoints = genTable)
